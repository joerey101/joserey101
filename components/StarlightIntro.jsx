 ...
 * @param {boolean} [props.showReplay]
 */
export default function StarlightIntro({ onComplete, showReplay = false }) {
  ...
  useEffect(() => {
    ...
    function draw(timestamp) {
      ...
      // ---- BLOOM SUPERNOVA ----
      // Bloom crece de 0 a 1 hasta BLOOM_PEAK
      const bloomPeak = clamp(pBloomUp, 0, 1);
      if (bloom) bloom.style.opacity = String(bloomPeak);

      // ---- CANVAS FADE A FLASH BLANCO -> FIN ----
      if (e >= T.BLOOM_PEAK) {
        const fadeValue = Math.max(0, 1 - easeOutCubic(norm(e, T.BLOOM_PEAK, T.BLOOM_END)));

        // Escondemos el canvas a partir del pico blanco para matar el fondo negro
        canvas.style.display = 'none';

        if (wrapperRef.current) {
          // El wrapper ahora es completamente blanco y su opacidad global desciende revelando el site suavemente
          wrapperRef.current.style.backgroundColor = '#ffffff';
          wrapperRef.current.style.opacity = String(fadeValue);
      ...
      if (e < T.BLOOM_END + 100) {
        ...
        onComplete?.();
        // Desmonta un poco más rápido para evitar huecos blancos (400ms en vez de 800ms)
